...
const AssignedTasks = () => {
  ...
  const { data: tasks, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/workitems/assigned-tasks');
      return data;
    },
    enabled: user?.globalRole !== 'User'
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between pb-6 border-b border-surface-200">
        <div>
          <h1 className="text-3xl font-bold text-surface-900 tracking-tight">Assigned Tasks</h1>
          <p className="text-surface-500 mt-1">Track the progress of tasks you have assigned to your team.</p>
        </div>
        <div className="flex items-center space-x-2 bg-primary-50 text-primary-700 px-4 py-2 rounded-lg font-medium">
          <BarChart2 size={20} />
          <span>{tasks?.length || 0} Total Active Assignments</span>
        </div>
      </div>
      ...
      <div className="card">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-xl font-medium text-surface-900">No Assigned Tasks</h3>
            <p className="text-surface-500 mt-2">You haven't assigned any active tasks to users yet.</p>
          </div>
        ) : (
                  ...
                  <tr key={item._id} onClick={() => openTaskDetail(item)} className="hover:bg-surface-50 transition-colors group cursor-pointer">
                    <td className="p-4 w-[60%]">
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="w-full max-w-[85%]">
                          <div className="font-semibold text-surface-900 group-hover:text-primary-600 transition-colors truncate">{item.title}</div>
                          <div className="text-sm text-surface-500 mt-1 line-clamp-1 truncate">{stripHtml(item.description)}</div>
                        </div>
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="flex items-center space-x-2">
                        <div className="w-8 h-8 rounded-full bg-primary-100 text-primary-700 flex items-center justify-center font-bold text-xs ring-2 ring-white shrink-0">
                          {item.assigneeId?.name?.charAt(0).toUpperCase() || '?'}
                        </div>
                        <span className="font-medium text-surface-700 whitespace-nowrap">{item.assigneeId?.name || 'Unassigned'}</span>
                      </div>
                    </td>
                    <td className="p-4">
                      <span className="inline-flex items-center space-x-1 text-sm bg-surface-100 text-surface-700 px-2.5 py-1 rounded-md font-medium whitespace-nowrap">
                        <Folder size={14} />
                    ...
                    <td className="p-4 w-[15%]">
                      <div className="flex flex-col space-y-2 items-start" onClick={(e) => e.stopPropagation()}>
                        <span className={`px-2.5 py-1 rounded-full text-xs font-bold border whitespace-nowrap ${getPriorityColor(item.priority)}`}>
                          {item.priority}
                        </span>
                        <select 
                          value={item.status} 
                          onChange={(e) => updateStatus.mutate({ itemId: item._id, status: e.target.value })}
                          className={`text-sm rounded-md border-surface-200 cursor-pointer shadow-sm focus:ring-primary-500 focus:border-primary-500 w-full ${
